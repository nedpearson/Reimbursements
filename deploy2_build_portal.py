...
def build(bills_folder=None, progress=print):
    ...
    # aim for ~6 MB per volume (fewer, still-loadable volumes) and never more than 6 volumes
    per=max(1,int(n*6.0/sz)) if sz else n
    per=max(per, -(-n//6))          # cap total volumes at 6
    volmap={}; volno=0; p=0
    ...
    tpl=tpl.replace('__CREDITNOTE__', (('after $__CREDITS__ already paid to %s is credited'%lindsey_name) if subtract else ('includes the direct payments/advances %s made to %s — see the Advances category below'%(ned_name,lindsey_name))))
